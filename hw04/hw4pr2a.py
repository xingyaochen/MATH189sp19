...
def grad_logreg(X, y, W, reg=0.0):
	...
	np.matmul(X.T, (sigmoid(X @ W) - y)) + reg * W
	inner = sigmoid(np.matmul(X, W)) - y
	grad = np.matmul(np.transpose(X), inner) + reg * W
	"*** END YOUR CODE HERE ***" 
	return grad
...
def NLL(X, y, W, reg=0.0):
	...
	temp = np.multiply(y, np.log(mu)) + np.multiply((1.0 - y), np.log(1.0 - mu))
	nll = -np.sum(temp) + reg/2 * np.power(np.linalg.norm(W), 2)
	"*** END YOUR CODE HERE ***"
	return nll
